...
# --- FUNCIÓN AUXILIAR: OBTENER FECHA SEGURA ---
def obtener_fecha_segura():
    """Obtiene fecha en formato colombiano, con fallback a inglés si falla."""
    try:
        if BOGOTA_TZ:
...
def generar_comprobante(data, config):
    ...
    try:
        ...
        raise FileNotFoundError(f"Plantilla no encontrada: {template_path}. Error: {e}")
        ...
        for campo, texto in datos.items():
            if campo in styles:
                ...
                try:
                    ...
                    # Fallback con fuente predeterminada
                    fallback_font = ImageFont.load_default()
                    draw.text(style["pos"], str(texto), font=fallback_font, fill=style["color"])
